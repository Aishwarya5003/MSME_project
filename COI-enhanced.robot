...
${endpoint_url}=    /cv/v1/cin_enhanced
${file_path}=   C:\\Users\\Aishwarya\\PycharmProjects\\APIAutomation\\data\\CIN-enhanced.csv
${client-username}=    526526315047
${client-password}=    BI9WnuOcxLBKKgPEB4qtLdADGc9PH0d5
${MAX_RESPONSE_TIME}    20000  # in milliseconds
${MIN_RESPONSE_TIME}    3000    # in milliseconds
${json_schema_file}=    C:\\Users\\Aishwarya\\PycharmProjects\\APIAutomation\\Json_schema\\COI services\\Json_schema COI enhanced.json
...
Send Post Request And Validate
    ...
    ${expected_code}=    Get From Dictionary    ${expected_case_json}    http_response_code
    ...
    FOR    ${i}    IN RANGE    ${retry_limit}
        ...
        # --- No retry if expected response = 503 ---
        Run Keyword If    ${expected_code} == 503    Exit For Loop

        # --- Retry only if ACTUAL = 503 ----
        Run Keyword If    ${response.status_code} != 503    Exit For Loop

        Log To Console    Unexpected 503 received → retrying...
        Sleep    3s
    END

    # Fail if still 503 but expected is not 503
    Run Keyword If    ${response.status_code} == 503 and ${expected_code} != 503
    ...    Fail    API returned unexpected 503 after ${max_retries} retries
